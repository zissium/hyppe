...
class SelectCoreImport {
	...
	public function import_action() {
						...
						case 'content':
							...
							if($content_start) {
								if (!SelectCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									qodef_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'qodef-cpt'));
									exit;
								}
							}
			...
			qodef_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $mixtape_widget_id => $mixtape_widget_data) {
				update_option('widget_' . $mixtape_widget_id, $mixtape_widget_data);
			}
	...
	public function import_sidebars_widgets( $file ) {
		$mixtape_sidebars = get_option( "sidebars_widgets" );
		unset( $mixtape_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$mixtape_sidebars = array_merge( (array) $mixtape_sidebars, (array) $data['sidebars'] );
			unset( $mixtape_sidebars['wp_inactive_widgets'] );
			$mixtape_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $mixtape_sidebars );
			$mixtape_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $mixtape_sidebars );
			return true;
		} else {
			return false;
		}
    ...
    public function import_settings_pages( $file ) {
        ...
        $new_ids = get_transient( '_qodef_core_imported_posts' );
        $fields_status = true;
        ...
        if($pages) {
            ...
            if (!$fields_status) {
                $this->set_status('error');
                $this->set_message(esc_html__('Problem occurred during settings pages import', 'qodef-cpt'));
            } else {
                $this->set_status('success');
                $this->set_data('type', 'options');
                $this->set_message(esc_html__('Settings pages imported successfully', 'qodef-cpt'));
            }
        } else {
            $this->set_status('error');
            $this->set_message(esc_html__('File doesn\'t exist', 'qodef-cpt'));
        }
    }
...
//
		...
		$mixtape_import = new WP_Import();
		set_time_limit( 0 );

		$mixtape_import->fetch_attachments = $attachments;
		$returned_value                  = $mixtape_import->import( $file . $xml );
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = qodef_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					qodef_core_ajax_status( 'success', '', $html);
				} else {
					qodef_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'qodef-cpt' ), '');
				}
			}
		}
