...
define( 'EOS_SCFM_PLUGIN_VERSION','0.1.7' );
...
function eos_scfm_post_content_replacement(){
	...
	if( isset( $_SERVER['REQUEST_URI'] ) && false !== strpos( $_SERVER['REQUEST_URI'],'-mobile' ) ){
		$desktop_post_id = eos_scfm_related_desktop_id( $post-> ID );
		if( $desktop_post_id > 0 && function_exists( 'is_preview' ) && !is_preview() ){
	...
	if( $mobile_post_id > 0 ){
		...
		if( $mobile_post && is_object( $mobile_post ) && 'trash' !== $mobile_post->post_status ){
			...
			add_action( 'wp_footer','eos_scfm_wp_footer_actions' );
		}
	}
}
...
function eos_scfm_wp_footer_actions(){
	if( isset( $GLOBALS['desktop_id'] ) && isset( $GLOBALS['desktop_id'] ) ){
	?>
	<!-- Specific Content For Mobile replaced the desktop content with the mobile content on this page (Desktop ID: <?php echo esc_attr( $GLOBALS['desktop_id'] ); ?>; Mobile ID: <?php echo esc_attr( $GLOBALS['mobile_id'] ); ?> --!>
	<?php
	}
}
...
function eos_scfm_is_tablet_mobile(){
	return !defined( 'SCFM_EXCLUDE_TABLETS' ) || !SCFM_EXCLUDE_TABLETS;
}
