...
class QodefTwitterApi {
	...
	public function obtainRequestToken() {
		...
		if(is_wp_error($response)) {
			...
			$responseObj->message = esc_html__('Internal WP error', 'select-twitter-feed');
		} else {
			...
			if(!empty($responseBody)) {
				...
				if((is_array($responseParsed) && count($responseParsed)) &&
					...
					if(!empty($responseObj->redirectUrl)) {
						$responseObj->status = false;
						$responseObj->message = esc_html__('Redirect URL couldn\t not be generated', 'select-twitter-feed');
					} else {
					...
					$responseObj->status = false;
					$responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'select-twitter-feed');
				}
			}
		}
		...
		if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
			...
			if(is_wp_error($response)) {
				...
				if(is_array($responseParsed) && count($responseParsed)
					...
					$responseObj->status = true;
					$responseObj->message = esc_html__('Access token obtained', 'select-twitter-feed');
				}
			}
		} else {
			$responseObj->status = false;
			$responseObj->message = esc_html__('Authorize token and it\'s secret were not obtainer', 'select-twitter-feed');
		}
	...
	public function fetchTweets($userId = '', $count = '', $transient = array()) {
		...
		if(!$this->transientExists($transient)) {
			if($userId && $accessToken) {
				...
				if(is_wp_error($response)) {
					...
					if(isset($response['response']['code']) && $response['response']['code'] == 200) {
						...
						$responseObj->status = false;
						$responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'select-twitter-feed');
					}
				}
			} else {
				$responseObj->status = false;
				$responseObj->message = esc_html__('It seams like you haven\t connected with your Twitter account', 'select-twitter-feed');
			}
		} else {
			...
			if(!empty($transientContent)) {
				...
				$responseObj->status = false;
				$responseObj->message = esc_html__('Couldn\'t retreive content from database', 'select-twitter-feed');
			}
		}
	 ...
	 * @return stdClass response object containing status, message and data properties
	 */
	public function getTweetById($postId = '', $transient = array()) {
		$responseObj = new stdClass();
		...
		if(!$this->transientExists($transient)) {
			if($postId && $accessToken) {
				...
				$requestParams['oauth_signature'] = $this->generateSignature('https://api.twitter.com/1.1/statuses/lookup.json', 'GET', $requestParams, $accessTokenSecret);

				unset($requestParams['id']);

				$response = wp_remote_get('https://api.twitter.com/1.1/statuses/lookup.json?id='.$postId, array(
					'headers' => array(
						'Authorization' => $this->generateOAuthHeader($requestParams),
						'Content-Type' => 'application/x-www-form-urlencoded;charset=UTF-8'
					)
				...
				if(is_wp_error($response)) {
					$responseObj->status = false;
					$responseObj->message = esc_html__('Internal WP error', 'qodef-twitter-feed');
				} else {
					if(isset($response['response']['code']) && $response['response']['code'] == 200) {
						$responseObj->status = true;
						$responseObj->message = esc_html__('Ok', 'qodef-twitter-feed');
						$responseObj->data = json_decode(wp_remote_retrieve_body($response), true);

						$this->updateTransient($transient, $responseObj->data);
					} else {
						$responseObj->status = false;
						$responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'qodef-twitter-feed');
					}
				}
			} else {
				$responseObj->status = false;
				$responseObj->message = esc_html__('It seams like you haven\t connected with your Twitter account', 'qodef-twitter-feed');
			}
		} else {
			$transientContent = $this->getTransient($transient);
			if(!empty($transientContent)) {
				...
				$responseObj->status = false;
				$responseObj->message = esc_html__('Couldn\'t retreive content from database', 'qodef-twitter-feed');
			}
		}
