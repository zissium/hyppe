...
class QodefInstagramApi {
	...
	private $apiURL;
	/**
	 * @varQodefInstagramApi instance of current class
	 */
	private static $instance;

	/**
	 * Helper object
	 * @varQodefInstagramHelper
	 */
	private $helper;
	 ...
	 * @return string
	 *
	 * @seeQodefInstagramApi::buildCurrentPageURI()
	 */
	 ...
	 *
	 * @seeQodefInstagramApi::fetchData()
	 */
	 ...
	 * @return stdClass
	 *
	 * @seeQodefInstagramApi::getAccessToken()
	 */
	private function fetchData($limit = '', $tag = '', $transient = array('use_transient' => false)) {
		...
		if($this->useTransients($transient) && get_transient($transient['transient_name'])) {
			...
			if(is_wp_error($httpResponse) || (isset($httpBody['meta']['code']) && $httpBody['meta']['code'] !== 200)) {
				...
				$returnObject->message = esc_html__('Can\'t connect with Instagram', 'qodef-instagram-feed');
		...
		$returnObject->message = esc_html__('Success', 'qodef-instagram-feed');
	...
	public function getPostById($link = '') {
		...
		$api_url = "https://api.instagram.com/oembed?url=".$link;
	...
	public function getAccessToken() {
		...
		if(empty($this->code)) {
			$returnObject->status = 'error';
			$returnObject->message = esc_html__('User has\'nt connected with Instagram', 'qodef-instagram-feed');
			...
			if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
				$returnObject->status = 'error';
				$returnObject->message = esc_html__('Can\t connect with Instagram API', 'qodef-instagram-feed');
			...
			if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
				...
				$returnObject->status = 'error';
				$returnObject->message = esc_html__('Can\t get Instagram access token', 'qodef-instagram-feed');
		...
		//prepare return object and return it
		$returnObject->status = 'ok';
		$returnObject->message = esc_html__('Stored access token', 'qodef-instagram-feed');
